  ...
  while (Serial.available())
  {
    ...
    if (c == '\n' || c == '\r')
    {
      ...
      else if (cmdBuf[0] == 'l' && cmdBuf[1] == 'o' && cmdBuf[2] == 'g')
      {
        ...
        {
          long ms;
          if (sscanf(cmdBuf + 3, "%ld", &ms) == 1 && ms >= 10)
          {
            LOG_MS = (uint32_t)ms;
            LOG_ATIVO = true;
...
// PID
float Carrinho::pidAtualizar(float erro, float dt, float kp, float ki, float kd)
{
...
void Carrinho::modoCorrida(uint8_t, float erro, float dt)
{
  ...
  float omega = pidAtualizar(erro, dt, kp, ki, kd);
  if (INVERTER_OMEGA)
    omega = -omega;
...
void Carrinho::seguirLinhaStep(float kp, float ki, float kd, float vyPercent)
{
